...
interface HeaderProps {
    ...
    onMenuClick: () => void;
    locale?: string;
    dict: any; // 🔥 SÖZLÜK (DICT) PROPS OLARAK EKLENDİ
}

export function Header({ user, onMenuClick, locale = 'tr', dict }: HeaderProps) {
    ...
    // Seçili şube ismini bul (Yoksa Sözlükten 'Tüm Şubeler' al)
    const currentBranchName = selectedBranch
        ? branches.find(b => b.id === selectedBranch)?.name
        : (dict?.header?.all_branches || 'Tüm Şubeler (Genel)');
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <div className="text-left hidden sm:block">
                                {/* 🔥 DİL DESTEĞİ GELDİ: AKTİF ŞUBE */}
                                <p className="text-[10px] text-slate-500 font-bold uppercase">{dict?.header?.active_branch || 'AKTİF ŞUBE'}</p>
                        ...
                        <>
                            ...
                            <div className="absolute top-full right-0 mt-3 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                                <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                                    <h3 className="font-bold text-slate-900 dark:text-white">{dict?.header?.notifications || 'Bildirimler'}</h3>
                                    {unreadCount > 0 && <span className="bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded-full font-bold">{unreadCount} {dict?.header?.new || 'Yeni'}</span>}
                                </div>
                                <div className="max-h-[400px] overflow-y-auto">
                                    {notifications.length === 0 ? (
                                        <div className="p-8 text-center text-slate-500">
                                            <Bell className="w-8 h-8 mx-auto mb-2 opacity-20" />
                                            <p className="text-sm">{dict?.header?.no_notifications || 'Henüz bildiriminiz yok.'}</p>
                                        </div>
                                    ) : (
                            ...
                            <div className="absolute top-full right-0 mt-3 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div className="p-2">
                                    <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                                        <LogOut className="w-4 h-4" /> {dict?.header?.logout || 'Çıkış Yap'}
                                    </button>
                                </div>
                            </div>
                        </>
