...
export default function RegisterPage() {
    ...
    const [verificationSent, setVerificationSent] = useState(false); // 🔥 Doğrulama ekranı durumu
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 6. Otomatik Merkez Şube (Patronlar İçin)
            if (!isStaff) {
                try {
            ...
            // Maili gönder, oturumu kapat ve onay ekranını göster.
            await sendEmailVerification(user);
            await signOut(auth); // Kullanıcıyı sistemden atıyoruz ki onaylamadan giremesin.
            setVerificationSent(true); // Onay ekranını aç
    ...
    const handleSocialLogin = async (providerName: 'google' | 'apple') => {
        // Sosyal medyada mail genellikle onaylı gelir, o yüzden burayı şimdilik aynı bırakabiliriz
        // veya oraya da kontrol ekleyebiliriz. Şimdilik manuel kayıt odaklıyız.
        setLoading(true);
        setError('');
        try {
            ...
            // ... (Sosyal login işlemleri aynen devam eder)
            // Sosyal login genelde verified email döndürür.
    ...
    return (
        <div className="flex min-h-screen w-full bg-white dark:bg-slate-950 font-sans selection:bg-blue-100 dark:selection:bg-blue-900">
            ...
            {/* SAĞ TARAF (FORM VEYA DOĞRULAMA EKRANI) */}
            <div className="w-full lg:w-[45%] flex flex-col items-center px-8 lg:px-12 bg-white dark:bg-slate-950 relative overflow-y-auto h-screen py-12">
                ...
                {/* 🔥 EĞER MAİL GÖNDERİLDİYSE BU EKRANI GÖSTER 🔥 */}
                {verificationSent ? (
                    <div className="max-w-[440px] w-full flex flex-col items-center justify-center h-full space-y-6 animate-in fade-in zoom-in duration-500 text-center">
                        ...
                        <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Mailinizi Kontrol Edin</h2>
                        <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 w-full">
                            <p className="text-slate-600 dark:text-slate-300 mb-4">
                                <span className="font-bold text-slate-900 dark:text-white">{formData.email}</span> adresine bir doğrulama bağlantısı gönderdik.
                            </p>
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                Hesabınızı aktif etmek ve sisteme giriş yapmak için lütfen gelen kutunuzdaki bağlantıya tıklayın. Spam/Gereksiz klasörünü kontrol etmeyi unutmayın.
                            </p>
                        </div>
                        <Link href="/login" className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 transition-all active:scale-95">
                            Giriş Yap Sayfasına Dön
                        </Link>
                    </div>
                ) : (
                    // 🔥 MAİL GÖNDERİLMEDİYSE FORMU GÖSTER (Eski Kod) 🔥
