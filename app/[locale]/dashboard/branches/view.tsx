...
export default function BranchesView({ dict }: { dict: any }) {
    ...
    const makeHeadquarters = async (branch: any) => {
        ...
        if (!confirm(`${branch.name} -> ${dict.branches.make_hq}?`)) return;
        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <Store className="text-blue-600 w-7 h-7" /> {dict.branches.title}
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">{dict.branches.subtitle}</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                    ...
                    >
                        ...
                        {isLimitReached ? dict.branches.limit_full : dict.branches.add_new}
                    </button>
                </div>
            </div>
            ...
            {isLimitReached && (
                <div className="bg-gradient-to-r from-red-50 to-white dark:from-red-900/10 dark:to-slate-900 border border-red-200 dark:border-red-900/30 p-4 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-red-700 dark:text-red-400">{dict.branches.limit_alert_title}</h3>
                        <p className="text-xs text-red-600 dark:text-red-500/80 mt-1">{dict.branches.limit_alert_desc}</p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {branches.length === 0 ? (
                    <div className="col-span-full text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <h3 className="font-bold text-lg text-slate-900 dark:text-white">{dict.branches.no_branches}</h3>
                        <p className="text-slate-500 text-sm mt-1 mb-6">{dict.branches.subtitle}</p>
                        ...
                        <div key={branch.id} className={`bg-white dark:bg-slate-800 rounded-2xl border p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative ${branch.isHeadquarters ? 'border-blue-500 ring-1 ring-blue-500/20' : 'border-slate-200 dark:border-slate-700'}`}>
                            ...
                            {branch.isHeadquarters && (
                                <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-bl-2xl rounded-tr-xl flex items-center gap-1 shadow-lg shadow-blue-600/20">
                                    <Crown className="w-3 h-3" /> {dict.branches.hq_badge}
                                </div>
                            )}
                            ...
                            <div className="space-y-3">
                                ...
                                {!branch.isHeadquarters && (
                                    ...
                                    >
                                        <Crown className="w-3.5 h-3.5" /> {dict.branches.make_hq}
                                    </button>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl p-0 shadow-2xl border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 overflow-hidden">
                        <div className="p-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                <Plus className="w-5 h-5 text-blue-600" /> {dict.branches.modal_title}
                            </h2>
                        ...
                        <div className="p-6">
                            <form onSubmit={handleAddBranch} className="space-y-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">{dict.branches.form_name}</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">{dict.branches.form_city}</label>
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">{dict.branches.form_district}</label>
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">{dict.branches.form_address}</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">{dict.staff.form_phone}</label>
                                ...
                                <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800 cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors" onClick={() => setNewBranch(p => ({ ...p, isHeadquarters: !p.isHeadquarters }))}>
                                    ...
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-200 select-none">{dict.branches.is_hq}</span>
                                </div>
                                ...
                                <button disabled={submitting} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center gap-2 mt-2">
                                    {submitting ? <Loader2 className="animate-spin w-5 h-5" /> : dict.branches.btn_save}
                                </button>
                            </form>
                        </div>
                    </div>
