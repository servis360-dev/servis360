...
export default function TicketChatView({ dict, id, locale }: { dict: any, id: string, locale: string }) {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                ...
                // Bilet Bilgisi
                const ticketRef = doc(db, 'artifacts', 'servis-360-live', 'public', 'data', 'tickets', id);
                onSnapshot(ticketRef, (doc) => setTicket(doc.data()));

                // Mesajlar
                const q = query(
                    collection(db, 'artifacts', 'servis-360-live', 'public', 'data', 'tickets', id, 'messages'),
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            await addDoc(collection(db, 'artifacts', 'servis-360-live', 'public', 'data', 'tickets', id, 'messages'), {
                text: newMessage,
                senderId: user.uid,
            ...
            await updateDoc(doc(db, 'artifacts', 'servis-360-live', 'public', 'data', 'tickets', id), {
    ...
    const closeTicket = async () => {
        if (confirm(dict.support.detail.confirm_close)) {
            await updateDoc(doc(db, 'artifacts', 'servis-360-live', 'public', 'data', 'tickets', id), {
                status: 'closed'
            });
        }
    ...
    const getStatusLabel = (status: string) => {
        switch (status) {
            case 'open': return dict.support.detail.status_open;
            case 'closed': return dict.support.detail.status_closed;
            case 'answered': return dict.support.detail.status_answered;
            default: return status;
        }
    };

    if (!ticket) return <div className="p-8 text-center text-slate-500">{dict.common.loading}</div>;

    return (
        <div className="h-[calc(100vh-100px)] flex flex-col bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in fade-in">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">{dict.support.detail.label_ticket_no}: #{id.substring(0, 6).toUpperCase()}</p>
                    </div>
                </div>
                {ticket.status !== 'closed' && (
                    <button onClick={closeTicket} className="text-xs font-bold text-slate-500 hover:text-red-500 border border-slate-300 hover:border-red-500 px-3 py-1.5 rounded-lg transition-all flex items-center gap-1">
                        <Lock className="w-3 h-3" /> {dict.support.detail.btn_close}
                    </button>
                )}
            </div>
                    ...
                    <div key={msg.id} className={`flex ${msg.senderId === user?.uid ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] rounded-2xl p-4 shadow-sm relative ${msg.senderId === user?.uid
                            ...
                            <p className={`text-[10px] mt-2 text-right ${msg.senderId === user?.uid ? 'text-blue-200' : 'text-slate-400'}`}>
                                {msg.createdAt?.seconds ? new Date(msg.createdAt.seconds * 1000).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' }) : '...'}
                            </p>
                        </div>
                    </div>
                ))}
            ...
            {ticket.status !== 'closed' ? (
                <form onSubmit={handleSend} className="p-4 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex gap-2">
                    <input
                        ...
                        placeholder={dict.support.detail.placeholder_reply}
                        value={newMessage}
                ...
                <div className="p-4 text-center bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 text-slate-500 text-sm font-medium">
                    {dict.support.detail.text_ticket_closed}
                </div>
            )}
        </div>
    );
