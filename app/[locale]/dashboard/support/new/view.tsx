...
export default function NewTicketView({ dict }: { dict: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            alert(dict.support.new.alert_error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{dict.support.new.title}</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
                ...
                <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{dict.support.new.label_subject}</label>
                    <input
                        required
                        ...
                        placeholder={dict.support.new.placeholder_subject}
                ...
                <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{dict.support.new.label_priority}</label>
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        >
                            {dict.support.new.priority_normal}
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            <AlertCircle className="w-4 h-4" /> {dict.support.new.priority_high}
                        </button>
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{dict.support.new.label_message}</label>
                    <textarea
                        required
                        rows={6}
                        ...
                        placeholder={dict.support.new.placeholder_message}
                ...
                >
                    {loading ? <Loader2 className="animate-spin w-5 h-5" /> : <><Send className="w-5 h-5" /> {dict.support.new.btn_submit}</>}
                </button>
