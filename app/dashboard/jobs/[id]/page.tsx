...
// 🔥 ŞUBE BAĞLANTISI (Opsiyonel, şube adını göstermek için)
...
export default function JobDetailPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const unsubscribeAuth = auth.onAuthStateChanged(async (user) => {
            ...
            // 1. Hedef UID Belirle (Personel ise Patronun Verisine Bakar)
            ...
            if (profileSnap.exists()) {
                ...
                setSettings(data); // Firma ayarlarını kaydet
            ...
            // 2. İş Detayını Dinle
            const jobRef = doc(db, 'artifacts', 'servis-360-live', 'users', ownerId, 'jobs', params.id);
            const unsub = onSnapshot(jobRef, (docSnap) => {
                if (docSnap.exists()) {
                    setJob({ id: docSnap.id, ...docSnap.data() });
                } else {
                    alert("İş bulunamadı veya silinmiş!");
    ...
    const handleStatusChange = async (newStatus: string) => {
        ...
        // Hedef ID'yi tekrar bulmak yerine job'ın sahibini veya mevcut mantığı kullan
        // Basitlik adına mevcut kullanıcı üzerinden profil kontrolü yapılabilir ama
        // job verisinde `createdBy` varsa oradan yürüyebiliriz.
        // Ancak en garantisi yine profil kontrolü ile ownerId bulmaktır.
        // Hızlı çözüm: job'ı okuduğumuz path'e yazmak lazım.
        // Buradaki state karmaşasını önlemek için job.id üzerinden direkt güncelleme yapalım.
        // Not: Burada user.uid kullanırsak ve personel isek hata alırız.
        // Bu yüzden yukarıdaki ownerId mantığını buraya da taşımalıyız veya job state'ine ownerId eklemeliyiz.
        // Şimdilik okuma başarılıysa yazma da aynı yoldan olmalı.
        ...
        await updateDoc(doc(db, 'artifacts', 'servis-360-live', 'users', ownerId, 'jobs', params.id), {
            status: newStatus
        });
    };
    ...
    const handleDelete = async () => {
        if (confirm("Bu kaydı kalıcı olarak silmek istiyor musunuz?")) {
            ...
            await deleteDoc(doc(db, 'artifacts', 'servis-360-live', 'users', ownerId, 'jobs', params.id));
            router.push('/dashboard/jobs');
        }
    };
    ...
    const sendWhatsapp = () => {
        ...
        // customerName kullanılıyor
        const customerName = job.customerName || job.customer || 'Müşteri';
        let msg = '';

        if (job.status === 'completed') {
            msg = `Sayın ${customerName}, ${job.device} cihazınızın işlemleri tamamlanmıştır. Teslim alabilirsiniz. Ücret: ${job.price} TL.`;
        } else {
            msg = `Sayın ${customerName}, ${job.device} cihazınız servisimize kabul edilmiştir. Takip No: ${job.id.slice(0, 6).toUpperCase()}`;
        }
    ...
    const customerName = job.customerName || job.customer || 'İsimsiz Müşteri';
    ...
    return (
        <div className="max-w-5xl mx-auto pb-20 px-4 md:px-0">
            ...
            {/* Üst Header: Geri Dön + Başlık + Butonlar */}
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 no-print">
                <div className="flex items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                            <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white">Servis Kaydı</h1>
                        ...
                        <div className="flex gap-2 mt-1">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-bold border ${statusConfig[job.status]?.color}`}>
                ...
                {/* Buton Grubu: Mobilde Grid, Masaüstünde Flex */}
                <div className="w-full lg:w-auto grid grid-cols-3 sm:flex gap-2">
                    <button onClick={sendWhatsapp} className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-3 py-2.5 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 active:scale-95 transition-all text-xs sm:text-sm">
                        <MessageCircle className="w-4 h-4 sm:w-5 sm:h-5" /> <span>WhatsApp</span>
                    </button>
                    <button onClick={handlePrint} className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-3 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 active:scale-95 transition-all text-xs sm:text-sm">
                        <Printer className="w-4 h-4 sm:w-5 sm:h-5" /> <span>Yazdır</span>
                    </button>
                    <button onClick={handleDelete} className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-3 py-2.5 bg-red-50 text-red-600 border border-red-100 rounded-xl font-bold hover:bg-red-100 active:scale-95 transition-all text-xs sm:text-sm">
                        <Trash2 className="w-4 h-4 sm:w-5 sm:h-5" /> <span>Sil</span>
                    </button>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                            <div>
                                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Cihaz & Model</p>
                            ...
                            <div>
                                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Seri No / IMEI</p>
                                <p className="font-mono text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 px-2 py-1 rounded w-fit text-sm">
                                    {job.serialNo || '-'}
                                </p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Ekran Kilidi</p>
                                <p className="font-mono text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 px-2 py-1 rounded w-fit text-sm">
                                    {job.password || 'Yok'}
                                </p>
                            </div>
                            <div>
                                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Teslim Alınanlar</p>
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                            <p className="text-xs text-slate-400 font-bold uppercase mb-2">Arıza / Şikayet</p>
                        ...
                        <div className="mt-4">
                            <p className="text-xs text-slate-400 font-bold uppercase mb-2">Tekniker Notu</p>
                            <p className="text-sm text-slate-600 dark:text-slate-400 italic">
                                {job.note || 'Not girilmedi.'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl">
                                <p className="text-xs text-slate-400 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3" /> Kayıt Tarihi</p>
                                <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
                                    {job.createdAt?.toDate ? job.createdAt.toDate().toLocaleDateString('tr-TR') : 'Bugün'}
                                </p>
                            </div>
                            {job.completedAt && (
                                <div className="bg-green-50 dark:bg-green-900/10 p-3 rounded-xl border border-green-100 dark:border-green-900/20">
                                    <p className="text-xs text-green-600 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3" /> Tamamlanma</p>
                                    <p className="text-sm font-bold text-green-700 dark:text-green-300">
                                        {job.completedAt.toDate().toLocaleDateString('tr-TR')}
                                    </p>
                                </div>
                            )}
                        </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm sticky top-6">
                        <h3 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                            <Wrench className="w-5 h-5 text-orange-500" /> İşlem Yönetimi
                        </h3>

                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Durumu Güncelle</label>
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Ücret Durumu</label>
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-sm text-slate-600 dark:text-slate-400">Servis Ücreti</span>
                                    <span className="text-2xl font-black text-slate-900 dark:text-white">{job.price ? job.price : '0'} ₺</span>
                                </div>
                                {job.paymentStatus === 'paid' ? (
                                    <div className="w-full py-2 bg-green-100 text-green-700 rounded-lg text-center font-bold text-sm flex items-center justify-center gap-2">
                                        <CreditCard className="w-4 h-4" /> Ödeme Alındı
                                    </div>
                                ) : (
                                    <div className="w-full py-2 bg-slate-100 dark:bg-slate-700 text-slate-500 rounded-lg text-center font-bold text-sm flex items-center justify-center gap-2">
                                        <CreditCard className="w-4 h-4" /> Ödeme Bekliyor
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {/* --- YAZDIRILACAK FİŞ ALANI (GİZLİ & MASAÜSTÜ İLE AYNI) --- */}
            <div id="printable-area" className="hidden print:block p-8 max-w-[80mm] mx-auto border bg-white text-black font-sans text-xs">
                ...
                <div className="text-center border-b pb-4 mb-4 border-black border-dashed">
                    ...
                    <p className="text-xs mt-1">{settings.address || 'Adres Bilgisi Yok'}</p>
                ...
                <div className="text-[8px] text-center text-slate-600 mt-4 border-t pt-2 border-black border-dashed">
                    <p>Teslim tarihinden itibaren 3 ay içinde alınmayan cihazlardan firmamız sorumlu değildir.</p>
