...
export default function SupportPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'open': return <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1"><Clock className="w-3 h-3" /> Açık</span>;
            case 'answered': return <span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Cevaplandı</span>;
            case 'closed': return <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Kapalı</span>;
            default: return <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold">Bilinmiyor</span>;
        }
    };
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">
                        {isAdmin ? 'Müşterilerden gelen tüm talepler.' : 'Sorunlarınızı ve sorularınızı buradan iletebilirsiniz.'}
                    </p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-12 text-center flex flex-col items-center">
                        ...
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white">Henüz talep yok</h3>
                        <p className="text-slate-500 text-sm mt-1 mb-6">Bir sorununuz varsa bize yazın.</p>
                        <Link href="/dashboard/support/new">
                            <button className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700">Talep Oluştur</button>
                        </Link>
                    </div>
                ) : (
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                                                <span>{new Date(t.createdAt?.seconds * 1000).toLocaleDateString('tr-TR')}</span>
                                                <span className={getPriorityColor(t.priority)}>{t.priority === 'high' ? 'Yüksek Öncelik' : t.priority === 'medium' ? 'Orta Öncelik' : 'Düşük Öncelik'}</span>
                                            </div>
                                        </div>
                                    </div>
