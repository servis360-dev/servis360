...
export default function CommunicationPage() {
    ...
    const getTypeStyles = (type: string) => {
        switch (type) {
            // Kurumsal Stiller
            case 'urgent': return { icon: AlertTriangle, classes: 'bg-red-50 border-red-200 text-red-900', label: 'ACİL DURUM', iconColor: 'text-red-600' };
            case 'announcement': return { icon: Megaphone, classes: 'bg-blue-50 border-blue-200 text-blue-900', label: 'DUYURU', iconColor: 'text-blue-600' };
            case 'general': return { icon: Info, classes: 'bg-white border-slate-200 text-slate-900', label: 'GENEL', iconColor: 'text-slate-500' };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        ...
                        {accountType === 'individual' ? 'Not Defterim' : 'Duyuru Panosu'}
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400">
                        {accountType === 'individual'
                            ? 'Kişisel notlarınızı ve hatırlatmalarınızı buraya yapıştırın.'
                            : 'Şirket içi önemli duyurular ve bilgilendirmeler.'}
                    </p>
                </div>
                ...
                {canPost && (
                    ...
                    >
                        <Plus className="w-5 h-5" /> {accountType === 'individual' ? 'Not Al' : 'Duyuru Yap'}
                    </button>
                )}
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 border-dashed">
                    ...
                    <p className="text-slate-500 font-medium">
                        {accountType === 'individual' ? 'Henüz not almadınız.' : 'Henüz bir duyuru yok.'}
                    </p>
                </div>
            ) : (
                            ...
                            return (
                                <div key={msg.id} className={`p-6 rounded-xl shadow-sm transition-transform hover:-translate-y-1 hover:shadow-lg relative group ${style.classes} dark:opacity-90`}>
                                    ...
                                    <span className="text-[10px] opacity-50 absolute bottom-3 right-3">
                                        {msg.createdAt?.seconds ? new Date(msg.createdAt.seconds * 1000).toLocaleDateString('tr-TR') : '-'}
                                    </span>
                                </div>
                            );
                        }
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                                            ...
                                            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">
                                                Gönderen: <span className="text-slate-900 dark:text-white font-bold">{msg.authorName}</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 border border-slate-200 dark:border-slate-700">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                                {accountType === 'individual' ? 'Yeni Not Ekle' : 'Yeni Duyuru Yap'}
                            </h2>
                        ...
                        <form onSubmit={handleSendMessage} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                    {accountType === 'individual' ? 'Not Rengi' : 'Mesaj Tipi'}
                                </label>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder={accountType === 'individual' ? 'Örn: Market Listesi' : 'Örn: Toplantı Hakkında'}
                                />
                            </div>
                            ...
                            >
                                {accountType === 'individual' ? 'Notu Yapıştır' : 'Duyuruyu Yayınla'}
                            </button>
                        </form>
                    </div>
                </div>
