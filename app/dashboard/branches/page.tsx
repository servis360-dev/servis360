...
export default function BranchesPage() {
    ...
    const getBranchLimit = () => {
        if (!profile) return 0;
        if (profile.customBranchLimit && profile.customBranchLimit > 0) return profile.customBranchLimit;
        if (['corporate', 'company'].includes(profile.accountType) || profile.role === 'corporate') return 5;
        return 1;
    };
        ...
        <div className="space-y-6 pb-20">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-700 p-4 rounded-xl flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-yellow-600 dark:text-yellow-500/80 mt-1">
                            ...
                            Daha fazla şube için ek paket satın alabilirsiniz.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {branches.length === 0 ? (
                    ...
                    branches.map(branch => (
                        <div key={branch.id} className={`bg-white dark:bg-slate-800 rounded-xl border p-5 shadow-sm hover:shadow-md transition-all group relative ${branch.isHeadquarters ? 'border-blue-500 ring-1 ring-blue-500/20' : 'border-slate-200 dark:border-slate-700'}`}>

                            {/* Merkez Rozeti */}
                            {branch.isHeadquarters && (
                                <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-bl-xl rounded-tr-xl flex items-center gap-1">
                            ...
                            <button
                                ...
                                className="absolute bottom-4 right-4 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                                title="Sil"
                            >
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${branch.isHeadquarters ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg min-h-[60px]">
                                ...
                                {!branch.isHeadquarters && (
                                    <button
                                        ...
                                        className="w-full mt-2 py-2 text-xs font-bold text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100"
                                    >
                                        <Crown className="w-3 h-3" /> Merkez Yap
                                    </button>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl p-6 shadow-2xl animate-in zoom-in-95">
                        ...
                        <form onSubmit={handleAddBranch} className="space-y-4">
                            <div>
                                ...
                                <input required className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" placeholder="Örn: Kadıköy Şube" value={newBranch.name} onChange={e => setNewBranch({ ...newBranch, name: e.target.value })} />
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Şehir</label>
                                    <input required className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" placeholder="İstanbul" value={newBranch.city} onChange={e => setNewBranch({ ...newBranch, city: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">İlçe</label>
                                    <input required className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" placeholder="Kadıköy" value={newBranch.district} onChange={e => setNewBranch({ ...newBranch, district: e.target.value })} />
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Açık Adres</label>
                                <textarea required rows={2} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" value={newBranch.address} onChange={e => setNewBranch({ ...newBranch, address: e.target.value })} />
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Telefon</label>
                                    <input required type="tel" className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" placeholder="0212..." value={newBranch.phone} onChange={e => setNewBranch({ ...newBranch, phone: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Yetkili E-Posta</label>
                                    <input type="email" required className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" placeholder="yonetici@sirket.com" value={newBranch.managerEmail} onChange={e => setNewBranch({ ...newBranch, managerEmail: e.target.value })} />
                                </div>
                            </div>

                            <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800 cursor-pointer" onClick={() => setNewBranch(p => ({ ...p, isHeadquarters: !p.isHeadquarters }))}>
                                <div className={`w-5 h-5 rounded border flex items-center justify-center ${newBranch.isHeadquarters ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-300'}`}>
                                    {newBranch.isHeadquarters && <CheckCircle2 className="w-3.5 h-3.5" />}
                                </div>
                                <span className="text-sm font-bold text-slate-700 dark:text-slate-200">Bu şubeyi MERKEZ olarak ayarla</span>
                            </div>

                            <button disabled={submitting} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center justify-center gap-2">
                                {submitting ? <Loader2 className="animate-spin w-5 h-5" /> : 'Kaydet'}
                            </button>
                        </form>
                    </div>
                </div>
