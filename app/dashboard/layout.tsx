...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
            ...
            if (!currentUser) {
                ...
                const unsubProfile = onSnapshot(doc(db, 'artifacts', 'servis-360-live', 'users', currentUser.uid, 'users', 'profile'), (docSnap) => {
                    if (docSnap.exists()) {
                        ...
                        // 👇 GÜNCELLENDİ: Profil yoksa aboneliğe yönlendir (Doğru yola)
    ...
    useEffect(() => {
        if (!loading && profile) {
            ...
            // 👇 GÜNCELLENDİ: Lisans bitince doğru adrese yönlendir
            if (profile.role !== 'admin' && isLicenseExpired && pathname !== '/dashboard/subscription') {
                router.push('/dashboard/subscription');
            }
        }
        ...
        <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
            ...
            <div className={`flex-1 flex flex-col transition-all duration-300 ${isLicenseValid ? 'ml-0 md:ml-64' : 'w-full'}`}>
                ...
                <main className="flex-1 p-4 md:p-6 overflow-y-auto">
                    {/* 👇 GÜNCELLENDİ: Pathname kontrolü doğru adrese göre yapıldı */}
                    {!isLicenseValid && pathname !== '/dashboard/subscription' ? (
                        <div className="flex flex-col items-center justify-center h-full text-center space-y-4 animate-in fade-in zoom-in duration-500 min-h-[80vh]">
                            ...
                            <div className="pt-4">
                                {/* 👇 GÜNCELLENDİ: Buton linki doğru adrese yönlendiriyor */}
